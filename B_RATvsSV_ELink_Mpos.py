...
def FDR(PSM_mf,fdrv):
    ...
    return PSM_mr
def PSM_sele(PSM_mf,pv,rt,pdes):
    PSM_mr=[]
    ct1=0
    ct2=0
    zt=0
    for psm in PSM_mf:
        zt+=1
        if psm[10] < pv and psm[1]>=rt and psm[2]>=pdes:
            PSM_mr.append(psm)
            ct1+=1
        else:
            ct2+=1
    print('get ',ct1,' remove ',ct2,' total ',zt)
    return PSM_mr
def InitB(msids):
    ...
    link=[593.28225]
    ppm2=10#ppm of ms2
    maxms2charge=3#max number of charges been considered in MS2
    r_score_list=[]
    rs_VMms2_list=[]
    rp_score_list=[]
    rPDE_score_list=[]
...
if __name__=='__main__':
    ...
    f=file('scanconb','rb')#load the combination file from A
    try:
        for i in range(zzcl):
            zzc=pickle.load(f)
            sidd.append(zzc)
    ...
    for recorda in SeqIO.parse(rfhpro,'fasta'):
        ...
        for recordb in SeqIO.parse(rfhpep,'fasta'):
            ...
            if len(pabind) == 0:
                ...
                Savetxtdata(Protein_d,'Protein.txt')
        rfhpep.close()
        #end2 = datetime.datetime.now()
        #print(end2-begin2)
    end = datetime.datetime.now()
